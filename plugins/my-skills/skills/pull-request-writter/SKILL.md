...
description: ALWAYS use this skill when creating or updating pull requests — never create or edit a PR directly without it. Trigger on any create PR, open PR, submit PR, make PR, update PR, push and create PR, or prepare changes for review task.
---
...
If there are uncommitted changes (modified, added, or untracked files that should be included), invoke the `promptly-coding:commit` skill first to commit them properly.
...
Before write the PR, check if we need can create more than one PR for all changes in the same branch. Small PRs are easier to review and merge.
However, this is only possible if the changes are related to the same concern. If the changes are unrelated, create a separate PR for each change.
...
## Discussion
- Explanation of what was done and the the reason behind it
- If necessary, create a mermaid fluxogram to explain the code flow or any other complexity

## Test plan
- Explain the test suit (just if we have one)
```
